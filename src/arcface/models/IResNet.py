...
def _iresnet(arch, block, layers, pretrained, progress, **kwargs):
    model = IResNetModel(block, layers, **kwargs)
    if pretrained:
        raise ValueError()
    return model
...
class IResNet(L.LightningModule):
    def __init__(self, file_path=None, input_size=112):
        ...
        if file_path:
            print("IResNet weights loaded")
            self.model = torch.load(file_path)
        else:        
            self.model = iresnet18(pretrained=None)
